;(function () {
    var _$ = function (_this) {
        return typeof _this.css == 'function' ? _this : $(_this);
    };
// 获取当前时间
    function now() {
        return +new Date();
        ...
        step: function(self) {
            ...
            var fn = function(){
                if (+new Date > self.end) {
                    ...
                    //x 每一步的X轴的位置
                    x = self.driftX * ((+new Date - self.begin) / self.duration);
                    //每一步的Y轴的位置y = a*x*x + b*x + c;   c==0;
                    y = self.curvature * x * x + self.b * x;
                
                    self.domove(x, y);
                    if (typeof ops.stepCallback === 'function') {
                        ops.stepCallback.call(self);
                    }
